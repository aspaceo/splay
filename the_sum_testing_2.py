...
for i in range(n):
    ...
    if line[0] == '+':
        ...
        if delete_value in tree:
        	print('delete ' + str(delete_value))
        	tree.delete(key = delete_value)
        else:
        	print('attempted to delete ' + str(delete_value))
    elif line[0] == '?':
        ## is this mother in the tree  ----------------------------------------
        find_value = ((last_sum_result + int(line[1])) % MODULO)
        find_value = find_value in tree
        if find_value:
            print("Found " + str(((last_sum_result + int(line[1])) % MODULO)))
        else:
            print("Not found " + str(((last_sum_result + int(line[1])) % MODULO)))
    else:
